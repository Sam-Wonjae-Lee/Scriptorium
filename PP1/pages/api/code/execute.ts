...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      const { stdout, stderr, compilerWarnings, timeout } = await executeCode(
        req,
        identifier
      );
      if (timeout) {
...
async function executeCode(req: NextApiRequest, identifier: string) {
  let executableArgs;
  let execProgram;
  ...
  const command =
    `${execProgram} ${executableArgs}` +
    (req.body.stdin ? ` < ${stdinPath}` : ``);
